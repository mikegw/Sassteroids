(function () {
  ...
  Sassteroid.prototype.avoid = function (that, maxDimensions) {
    if (that instanceof Sassteroid) {
      this.updateFutureVelocity(that, maxDimensions);
      this.calculateNextPos(maxDimensions);
      that.calculateNextPos(maxDimensions);
      while (this.willCollideWith(that, maxDimensions)) {
        [this, that].forEach(function (sassteroid) {
          sassteroid.move();
          sassteroid.calculateNextPos(maxDimensions);
        });
      }
    }
  };
  ...
  Sassteroid.prototype.updateFutureVelocity = function (that, maxDimensions) {
    var uSpeed = magnitude(this.vel);
    var vSpeed = magnitude(that.vel);
