(function () {
  ...
  var subtractVects = Utils.subtractVects = function (vect1, vect2) {
    return addVects(vect1, scale(vect2, -1));
  };
  ...
  var shortestVect = Utils.shortestVect = function(vect1, vect2, maxDimensions) {
    ...
    for (var i = 0; i < 2; i++) {
      ...
      for (var j = 0; j < vect2Alts.length; j++) {
        ...
        if (Math.abs(side) < Math.abs(shortestSide)) {
          shortestSide = side;
        }
      }
  ...
  Sassteroids.Utils.drawWithWrap = function (options) {
    var screenInfo = options.screenInfo;
    var pos = options.movingObject.pos;
    var radius = options.movingObject.radius;
    var drawLogic = options.drawLogic;
    
    pos = subtractVects(pos, screenInfo.screenPos);

    var planePos = [[], []];
    for (var i = 0; i < 2; i++) {
      planePos[i] = [
        pos[i] - screenInfo.gameDimensions[i], pos[i], pos[i] + screenInfo.gameDimensions[i]
      ];

      planePos[i] = planePos[i].filter(function (ord) {
        return (ord > -radius) && (ord < screenInfo.screenDimensions[i] + radius)
      });
    }
